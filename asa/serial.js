...
class UsbEndpointUnderlyingSource {
    ...
    constructor(device, endpoint, onError) {
        ...
        this.type = "bytes";
        this.autoAllocateChunkSize = 1024;
    }
    /**
    ...
    pull(controller) {
        (async () => {
            ...
            try {
                const result = await this.device_.transferIn(this.endpoint_.endpointNumber, this.endpoint_.packetSize);
                if (result.status != 'ok') {
                    controller.error(`USBin error: ${result.status}`);
                    this.onError_();
                }
                ...
                controller.error("In:"+error.toString());
                this.onError_();
            }
        })();
    }
...
class UsbEndpointUnderlyingSink {
    ...
    async write(chunk, controller) {
        try {
            ...
            controller.error("out:"+error.toString());
            this.onError_();
        }
    }
}
